...
def predict(image):
    ...
    model = models.detection.fasterrcnn_resnet50_fpn(pretrained=True,progress=True).to('cuda:1')
    #model = models.detection.fasterrcnn_mobilenet_v3_large_320_fpn(pretrained=True,progress=True)
    #model = models.detection.fasterrcnn_mobilenet_v3_large_320_fpn(pretrained=True,progress=True).to('cuda:1')
    ...
    batch_t = torch.unsqueeze(transform(img), 0).to('cuda:1')
